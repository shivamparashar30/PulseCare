...
export default function DiagnosticCenters() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      .from('profiles')
      .select('*, center:diagnostics_centers(*)')
      .eq('role', 'diagnostics')
  ...
  const updateStatus = async (id: string, status: 'approved' | 'rejected', reason?: string) => {
    ...
    setCenters(prev => prev.map(c => c.id === id ? { ...c, status, rejection_reason: reason } : c));
    if (selected?.id === id) setSelected({ ...selected, status, rejection_reason: reason });
    setRejectId(null);
    setRejectReason('');
  };

  const ct = (c: any) => Array.isArray(c.center) ? c.center[0] : c.center;

  const filtered = centers.filter(c => {
    const name = ct(c)?.center_name || c.full_name || '';
    const matchSearch = name.toLowerCase().includes(search.toLowerCase()) || c.email?.toLowerCase().includes(search.toLowerCase());
  ...
  const columns = [
    {
      ...
      render: (c: any) => (
        <div className="flex items-center gap-3">
          {ct(c)?.center_photo_url ? (
            <img src={ct(c).center_photo_url} alt="" className="w-8 h-8 rounded-lg object-cover" />
          ) : (
            <div className="w-8 h-8 rounded-lg bg-indigo-100 flex items-center justify-center text-indigo-700 font-semibold text-xs">
              {(ct(c)?.center_name || c.full_name || '?').charAt(0).toUpperCase()}
            </div>
          )}
          <div>
            <p className="font-medium text-gray-900">{ct(c)?.center_name || c.full_name || 'Unnamed'}</p>
            <p className="text-xs text-gray-500">{c.email}</p>
          </div>
        </div>
      ),
    },
    { key: 'reg_id', label: 'Reg. ID', render: (c: any) => ct(c)?.registration_id || '-' },
    { key: 'phone', label: 'Phone', render: (c: any) => ct(c)?.phone || c.phone || '-' },
    { key: 'address', label: 'Address', render: (c: any) => ct(c)?.address || '-' },
      ...
      <DataTable
        ...
        actions={(c: any) => (
          <div className="flex items-center gap-1.5 justify-end">
            ...
            {c.status !== 'rejected' && <button onClick={() => setRejectId(c.id)} className="p-1.5 text-gray-400 hover:text-red-600 rounded"><XCircle size={15} /></button>}
          </div>
        )}
      />
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 space-y-5 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            {ct(selected)?.center_photo_url && (
              <img src={ct(selected).center_photo_url} alt="Center" className="w-full h-40 object-cover rounded-xl" />
            )}

            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-gray-500">Center Name:</span> <span className="font-medium">{ct(selected)?.center_name || '-'}</span></div>
              ...
              <div><span className="text-gray-500">Phone:</span> <span className="font-medium">{ct(selected)?.phone || selected.phone || '-'}</span></div>
              <div><span className="text-gray-500">Registration ID:</span> <span className="font-medium">{ct(selected)?.registration_id || '-'}</span></div>
              <div><span className="text-gray-500">Address:</span> <span className="font-medium">{ct(selected)?.address || '-'}</span></div>
            </div>

            <div>
              <h3 className="text-sm font-semibold text-gray-700 mb-3">Verification Documents</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <DocCard url={ct(selected)?.government_id_url} label="Government ID" />
                <DocCard url={ct(selected)?.center_photo_url} label="Center Photo" />
                <DocCard url={ct(selected)?.registration_certificate_url} label="Registration Cert." />
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4" onClick={() => setRejectId(null)}>
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <textarea value={rejectReason} onChange={e => setRejectReason(e.target.value)}
              placeholder="e.g. Invalid registration ID, unclear document photos..." className="w-full p-3 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 resize-none" rows={3} />
