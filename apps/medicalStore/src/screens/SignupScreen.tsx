...
export default function SignupScreen({ onGoToLogin, onSignupComplete }: Props) {
  ...
  const [shopPhoto, setShopPhoto] = useState<DocState>({ uri: null, uploading: false });
  const [licenseCert, setLicenseCert] = useState<DocState>({ uri: null, uploading: false });
  ...
  const handleSignup = async () => {
    if (!storeName || !ownerName || !email || !password || !licenseNumber) {
      setError('Please fill all required fields');
      return;
    }
    if (!govId.uri || !shopPhoto.uri || !licenseCert.uri) {
    ...
    try {
      ...
      const { data: authData, error: authError } = await supabase.auth.signUp({
        email,
        password,
        options: { data: { full_name: ownerName, role: 'medical_store' } },
      });
      if (authError) {
      ...
      // 2. Save profile + store data via SECURITY DEFINER RPC (bypasses RLS)
      const { error: rpcError } = await supabase.rpc('create_store_signup', {
        p_user_id: userId,
        p_email: email,
      ...
      try {
        const [govIdUrl, shopPhotoUrl, licenseCertUrl] = await Promise.all([
          uploadDocument(userId, 'government_id', govId.uri!),
          uploadDocument(userId, 'shop_photo', shopPhoto.uri!),
          uploadDocument(userId, 'license_certificate', licenseCert.uri!),
        ]);

        // 5. Update records with document URLs
        await supabase.from('profiles').update({ avatar_url: shopPhotoUrl }).eq('id', userId);
        await supabase.from('medical_stores').update({
          government_id_url: govIdUrl,
          shop_photo_url: shopPhotoUrl,
          license_certificate_url: licenseCertUrl,
        }).eq('id', userId);
      } catch (uploadErr: any) {
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <TextInput style={styles.input} value={storeName} onChangeText={setStoreName} placeholder="e.g. HealthPlus Pharmacy" placeholderTextColor="#9CA3AF" />

        <Text style={styles.label}>Owner Name <Text style={styles.required}>*</Text></Text>
        <TextInput style={styles.input} value={ownerName} onChangeText={setOwnerName} placeholder="Full name of owner" placeholderTextColor="#9CA3AF" />

        <Text style={styles.label}>Email <Text style={styles.required}>*</Text></Text>
        <TextInput style={styles.input} value={email} onChangeText={setEmail} placeholder="store@example.com" placeholderTextColor="#9CA3AF" keyboardType="email-address" autoCapitalize="none" />
        ...
        <Text style={styles.section}>LICENSE DETAILS</Text>

        <Text style={styles.label}>Pharmacy License Number <Text style={styles.required}>*</Text></Text>
        <TextInput style={styles.input} value={licenseNumber} onChangeText={setLicenseNumber} placeholder="Drug License No." placeholderTextColor="#9CA3AF" />

        <Text style={styles.label}>Store Address</Text>
        <TextInput style={styles.input} value={address} onChangeText={setAddress} placeholder="Full store address" placeholderTextColor="#9CA3AF" multiline />

        <Text style={styles.section}>VERIFICATION DOCUMENTS</Text>

        <DocUpload label="Government ID (Aadhaar/PAN)" doc={govId} setDoc={setGovId} />
        <DocUpload label="Shop Photo" doc={shopPhoto} setDoc={setShopPhoto} />
        <DocUpload label="Pharmacy License Certificate" doc={licenseCert} setDoc={setLicenseCert} />
...
const styles = StyleSheet.create({
  ...
  section: { fontSize: 11, fontWeight: '700', color: '#059669', letterSpacing: 1.5, marginTop: 28, marginBottom: 12 },
  ...
  docPicker: { alignItems: 'center', padding: 20, borderWidth: 1.5, borderColor: '#A7F3D0', borderRadius: 10, borderStyle: 'dashed', backgroundColor: '#ECFDF5' },
  docPickText: { fontSize: 12, color: '#059669', marginTop: 6, fontWeight: '500' },
  ...
  submitBtn: { backgroundColor: '#059669', paddingVertical: 16, borderRadius: 14, alignItems: 'center', marginTop: 28 },
