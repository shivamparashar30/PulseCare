...
export default function App() {
  ...
  const checkAuth = async () => {
    ...
    const { data } = await supabase.from('profiles').select('*').eq('id', session.user.id).eq('role', 'doctor').single();
    ...
    if (data.status === 'approved') {
      ...
      const { data: dp } = await supabase.from('doctor_profiles').select('address_line1, city, state, pincode, latitude, longitude').eq('id', data.id).single();
      if (!dp?.address_line1 || !dp?.city || !dp?.state || !dp?.pincode || dp?.latitude == null) {
  ...
  const handleLogin = async (email: string, password: string) => {
    ...
    const { data: p } = await supabase.from('profiles').select('*').eq('id', data.user.id).eq('role', 'doctor').single();

    if (!p) {
      const { data: anyProfile } = await supabase.from('profiles').select('role').eq('id', data.user.id).single();
      await supabase.auth.signOut();
      if (anyProfile && anyProfile.role !== 'doctor') {
    ...
    if (p.status === 'approved') {
      const { data: dp } = await supabase.from('doctor_profiles').select('address_line1, city, state, pincode, latitude, longitude').eq('id', p.id).single();
      if (!dp?.address_line1 || !dp?.city || !dp?.state || !dp?.pincode || dp?.latitude == null) {
        setState('address_incomplete');
      } else {
        setState('home');
  ...
  if (state === 'loading') return <View style={styles.loadingContainer}><ActivityIndicator size="large" color="#2563EB" /></View>;
  ...
  return (
    <SafeAreaProvider>
      ...
      {state === 'address_incomplete' && <AddressCompletionScreen accentColor="#2563EB" role="Doctor" onSave={handleAddressSave} onLogout={handleLogout} />}
      {state === 'home' && <DoctorDashboard onLogout={handleLogout} profile={profile} />}
    </SafeAreaProvider>
  );
}
